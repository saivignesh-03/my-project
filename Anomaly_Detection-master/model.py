...
class VideoLearn():
    ...
    def learn(self, data, target, epochs):
        print('target train 0/1: {}/{}'.format(len(np.where(target == 0)
              [0]), len(np.where(target == 1)[0])))
        ...
        for epoch in range(epochs):
            ...
            for i, (b_x, b_y) in enumerate(train_loader):
                ...
                self.optimizer.step()
                total += b_y.shape[0]
                detected_anomaly += torch.count_nonzero(predicted).item()
                total_anomaly += torch.count_nonzero(batch_y).item()
                correct += (predicted == batch_y.long()).sum().item()
            print("epoch;", "accuracy;", "loss;", "detected anomaly;",
                  "total anomaly;", "difference learned")
            print(epoch, correct/total, running_loss/total, detected_anomaly,
                  total_anomaly, detected_anomaly - total_anomaly)
...
class VideoClassifier():
    ...
    def predict(self, input_data, label):
        test = Data.TensorDataset(torch.FloatTensor(
            input_data), torch.FloatTensor(label))
